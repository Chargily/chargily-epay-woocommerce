...
add_action( 'plugins_loaded', 'chargily_edahabia_init_gateway_class', 0 );
function chargily_edahabia_init_gateway_class() {
    //if condition use to do nothin while WooCommerce is not installed
    if ( ! class_exists( 'WC_Payment_Gateway' ) ) return;
    include_once('edahabia.php');
    // class add it too WooCommerce
    add_filter( 'woocommerce_payment_gateways', 'chargily_edahabia_add_gateway_class' );
    function chargily_edahabia_add_gateway_class( $methods ) {
        $methods[] = 'WC_Chargily_Edahabia_Gateway';
        return $methods;
    }

    class WC_Chargily_Edahabia_Gateway extends WC_Payment_Gateway
    {
        ...
        function __construct()
        {
            ...
            $this->id = 'edahabia';
            $this->icon = plugins_url( '../assets/edahabia-logo.png', __FILE__ );
            $this->has_fields = true;
            $this->method_title = 'EDAHABIA';
            $this->method_description = 'Payment using Edahabia card';
        ...
        public function init_form_fields()
        {
            $this->form_fields = array(
                'enabled' => array(
                    'title' => __('Enable / Disable', 'edahabia'),
                    'label' => __('Enable this payment gateway', 'edahabia'),
                    'type' => 'checkbox',
                    'default' => 'no',
                ),
                'title' => array(
                    'title' => __('Title', 'edahabia'),
                    'type' => 'text',
                    'desc_tip' => __('Payment method title', 'edahabia'),
                    'default' => __('EDAHABIA', 'edahabia'),
                    'custom_attributes' => array(
                        'required'       =>  true
                    ),
                ),
                'description' => array(
                    'title' => __('Description', 'edahabia'),
                    'type' => 'textarea',
                    'desc_tip' => __('sub title of this paiement method', 'edahabia'),
                    'default' => __('Paiement par la Carte EDAHABIA.', 'edahabia'),
                    'css' => 'max-width:450px;'
                ),
                'auth_key' => array(
                    'title' => __('API key', 'edahabia'),
                    'type' => 'password',
                    'desc_tip' => __('Get the API Key from your account profil in the platform ePay by Chargily.', 'edahabia'),
                    'custom_attributes' => array(
                        'required'       =>  true
                    ),
                ),
                'signature' => array(
                    'title' => __('Secret key', 'edahabia'),
                    'type' => 'password',
                    'desc_tip' => __('Get the Secret Key from your account profil in the platform ePay by Chargily.', 'edahabia'),
                    'custom_attributes' => array(
                        'required'       =>  true
                    ),
                ),
        ...
        public function validate_value_field( $value ) {
            if (isset( $value ) ){

                if ( $value > 100 || $value < 0) {

                    WC_Admin_Settings::add_error( esc_html__( 'the value of Edahabia discount '.$value . ' is not valid,must be between 1 and 99' ) );
//                    return 0;
                }
                else if ( $value == 100 ) {
